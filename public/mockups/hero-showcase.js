...
(() => {
  ...
  function openProject(project) {
    ...
    byId("detail-tags").replaceChildren(
      ...project.tags.map((tag) => element("span", "tag", tag)),
    );
    byId("detail-link").href = project.link;
    byId("detail-link").setAttribute(
      "aria-label",
  ...
  portfolioProjects.forEach((project, index) => {
    ...
    tags.append(
      ...project.tags.slice(0, 3).map((tag) => element("span", "tag", tag)),
    );
    ...
    body.append(
      element("div", "project-category", project.label),
      title,
  ...
  function applyFilters() {
    ...
    portfolioProjects.forEach((project, index) => {
      const matches =
        ...
        [project.title, project.label, project.description, ...project.tags]
          .join(" ")
          .toLowerCase()
          .includes(query);
    ...
    byId("result-count").textContent =
      `Showing ${count} of ${portfolioProjects.length} projects`;
